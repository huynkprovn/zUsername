...
namespace ZedShadow
{
    internal class Program
    {
		...
		private static float TargetRange;
        ...
        {     
			...
			autoIgnite();
			CloneCheck();
        	target = SimpleTs.GetTarget(TargetRange, SimpleTs.DamageType.Physical);
			...
			else if (Config.Item("farmKey").GetValue<KeyBind>().Active) 
			{
				Farm();
			}
        }
		...
		{        	
        	...
        	if (sender.IsValid && sender.Name.Contains("Zed_Base_R_buf_tell.troy")) 
        	{
        		isDead = true;
        	}
		}
 		...
 		private static void autoIgnite()
        {
 		...
 		private static void Swap()
 		{
 			...
 			if (UseSwap)
 			{
 				...
 				if (wClone != null && wClone.IsValid) 
 				{
 					wDist = target.Distance(wClone.Position);
 				}
 				else 
 				{
 					return;
        ...
        private static int CountEnemiesInObject(GameObject point ,float range)
        {
        	int ChampCount = 0;
        	foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.IsEnemy && enemy.IsValidTarget(750) && 
        	                                                             enemy.Distance(point.Position) <= range ))
        		ChampCount = ChampCount + 1 ;
            return ChampCount;
        }
 		...
 		private static void Fight()
 		{
 			...
 			if (Q.IsReady() && E.IsReady() && W.IsReady() && R.IsReady())
 			{
 				TargetRange = 1200;
 			}
 			else
 			...
 			if (!(target.HasBuff("JudicatorIntervention") || target.HasBuff("Undying Rage")))
 			{
 				...
 				if (!R.IsReady() || rClone != null )
 				{
 					...
 					{
 						if (!(NoWWhenUlt && ( (myHero.Spellbook.GetSpell(SpellSlot.R).Name == "ZedR2") || (rClone != null && rClone.IsValid) ) ))
 						{
 							if (MyMana > (WMana+EMana)) 
 							{
 			...
 			if (R.IsReady() && rClone != null && rSwap)
 				if (isDead && CountEnemies(myHero,250) > CountEnemiesInObject(rClone,250))
 				{	
 				    UseSwap = false;
 				    if (UsePacket) 
 				    {
 					...
 					if (myHero.Position == rClone.Position) 
 					{
 						UseSwap = true;
 					}
 				}
 			...
 			if (target.IsValid)
 			{
 				...
 				if (UltDmg >= target.Health)
 				{
 					if (myHero.Distance(target) < 1125 && myHero.Distance(target) > 750)
 					{
 						...
 						if (wClone != null && wClone.IsValid && rClone == null)
 						{
 		...
 		private static void Harass()
 		{
 			...
 			{
 				if (Q.IsReady() && myHero.Distance(target) < Q.Range) 
 				{
 					CastQ();
 				}
 				else if (E.IsReady() && myHero.Distance(target) < 280) 
 				{
 					CastE();
 				}
 				else if (wClone != null && wClone.IsValid)
 				{
 					CastE();
 					CastQClone();
 				}
 		...
 		{
 			var UsePacket = Config.Item("UsePacket").GetValue<bool>();
 			if (target.IsValid && Q.IsReady())  
 			{
 				if (target.Distance(myHero) < Q.Range || target.Distance(wClone.Position) < Q.Range || target.Distance(rClone.Position) < Q.Range)
 				{
 		...
 		{
 			var UsePacket = Config.Item("UsePacket").GetValue<bool>();
 			if (target.IsValid && Q.IsReady() && target.Distance(wClone.Position) < Q.Range)
 			{
 				Q.CastIfHitchanceEquals(target,HitChance.High,UsePacket);
 			}
 		}
		...
		{
        	var UsePacket = Config.Item("UsePacket").GetValue<bool>();
        		if (target.IsValid && (target.Distance(myHero) <= 280 || target.Distance(wClone.Position) <= 280 || target.Distance(rClone.Position) <= 280) )
        		{
        			if (UsePacket) 
        			{
        				Packet.C2S.Cast.Encoded(new Packet.C2S.Cast.Struct(0,SpellSlot.E)).Send();
 		...
 		private static void Farm()
        {
        	...
        	foreach (var minion in allMinions)
        	{
        		if (UseQ && Q.IsReady())
        		{
					var dmg = Damage.GetSpellDamage(myHero,minion,SpellSlot.Q);
					if (dmg >= minion.Health && myHero.Distance(minion) <= Q.Range) Q.Cast(minion,UsePacket);				
        		}
        		if (UseE && E.IsReady())
        		{
					var dmg = Damage.GetSpellDamage(myHero,minion,SpellSlot.E);
					if (dmg >= minion.Health && myHero.Distance(minion) <= 280)
					{
